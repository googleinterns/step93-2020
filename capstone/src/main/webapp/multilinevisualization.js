...
const y = d3.scaleLinear()
    .domain([0, d3.max(data.restaurantData, d => d3.max(d.clickData))]).nice()
...
// 'g' elements are containers for graphic and can group related graphics elements
svg.append("g")
    .call(xAxis);
 ...
 * the closest point to that position. Also greys out all lines but the one with
 * the closest point and displays the name of the restaurant the selected
 * line belongs to
...
function hover(svg, path) {
    ...
    function moved() {
        ...
        // Check both the point to the left and right of current position to see which is closer
        const i1 = d3.bisectLeft(data.dates, xm, 1);
        const i0 = i1 - 1;
        const i = xm - data.dates[i0] > data.dates[i1] - xm ? i1 : i0;

        // Now look through the different y-values, aka click data for each restaurant,
        // to find which is closest to the mouse's current y-position
        ...
        // Make every line greyed out except the one currently selected by the mouse
        path.attr("stroke", d => d === s ? null : "#ddd").filter(d => d === s).raise();

        // Translate the dot to be in the correct location on the screen based on the
        // dates array and the clickData of the selected restaurant
        dot.attr("transform", `translate(${x(data.dates[i])},${y(s.clickData[i])})`);
        // Label the line currently being hovered over with the current restaurant name
    ...
    function left() {
        // Allow the lines to go back to full color and hide the dot from the display
